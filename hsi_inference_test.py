...
class HSIEdgeProcessor:
    def __init__(self, yolo_weights, api_url, crop_height=480, crop_width=640,
        ...
        # ==========================================================
        # 🔒 与在线分选程序 (industrial_vision_cpp) 严格对齐的分割预处理参数。
        #    源: cuda_compute/SortingKernels.cu::HsiToYoloRgbKernel
        #        cuda_compute/YoloInferenceEngine.{h,cpp}
        #        sorting_config.json / SortingEngine.h 默认值
        #
        #    在线 YOLO 伪彩 = 对【原始 DN】做固定增益归一化 clip(raw/gain, 0, 1)，
        #    不做反射率校准、不做百分位拉伸、不涂黑边缘。波段 44/63/88，conf=0.3。
        #    ⚠️ 前提: envi 读到的 cube 必须与在线一致, 是【原始 DN】而非反射率;
        #       若是反射率([0,1]), gain=7000 会把整图压成 0, 需把 gain 改成 ~1.0。
        # ==========================================================
        ...
        self.rgb_bands = list(rgb_bands)              # 在线: yolo_rgb_r/g/b = 44/63/88
        self.expected_bands = expected_bands
        self.yolo_gains = list(yolo_gains)            # 在线: yolo_gain_r/g/b = 7000 (按 R/G/B 通道)
        self.conf_thresh = conf_thresh               # 在线: yolo_conf_threshold = 0.3
        self.iou_thresh = iou_thresh                 # 在线: yolo_iou_threshold = 0.45
        self.min_particle_area = min_particle_area   # 在线: hsi_min_particle_area = 10
        self.belt_margin = belt_margin
        print(f"🔗 [初始化] 目标服务器接口: {self.api_url}")


    def _normalize_fixed_gain(self, img_array, gains):
        """
        与在线 HsiToYoloRgbKernel 一致的固定增益归一化:
            pixel = clip(raw / gain, 0, 1) * 255
        - 逐像素独立, 不依赖任何全局统计 (与百分位拉伸不同)。
        - gains 按通道给 (对应 img_array 的通道顺序, 即 RGB 波段顺序)。
        - 不涂黑边缘 (在线无此步)。
        """
        img = img_array.astype(np.float32)
        res = np.empty_like(img)
        for i in range(img.shape[2]):
            g = gains[i] if gains[i] > 1e-6 else 1.0
            res[:, :, i] = np.clip(img[:, :, i] / g, 0.0, 1.0)
    ...
    def process_and_upload(self, hdr_path, batch_name):
        ...
        try:
            ...
            # ==========================================
            # 🌟 2. 制作 YOLO 视觉替身 (物理隔离) —— 与在线分选严格对齐
            #    在线: raw DN → clip(raw/gain,0,1) → 水平翻转 → resize。
            #    不做反射率、不做百分位拉伸、不涂黑边缘。
            # ==========================================
            rgb_raw = working_data_cube[:, :, self.rgb_bands]

            # 替身翻转：只把发给 YOLO 的替身做镜像，使其对齐物理气阀坐标 (在线 kFlipX=1)
            rgb_flipped = np.flip(rgb_raw, axis=1)

            # 固定增益归一化 (RGB 通道顺序) → 转 BGR 喂给 ultralytics
            rgb_8bit = self._normalize_fixed_gain(rgb_flipped, self.yolo_gains)
            bgr_img = cv2.cvtColor(rgb_8bit, cv2.COLOR_RGB2BGR)

            # ❌ 不再涂黑边缘: 在线 HsiToYoloRgbKernel 没有 margin 涂黑这一步。
            ...
            for start_y in start_y_list:
                ...
                results = self.yolo(crop_bgr, conf=self.conf_thresh, iou=self.iou_thresh,
            ...
            for label in range(1, num_labels):
                ...
                if area < self.min_particle_area:  # 在线: hsi_min_particle_area = 10
                    continue
                
                if self.belt_margin > 0:
                    if cx < self.belt_margin or cx > (self.crop_width - self.belt_margin):
                        continue
    ...
    def process_and_upload_bak(self, hdr_path, batch_name):
        ...
        try:
            ...
            for start_y in start_y_list:
                ...
                results = self.yolo(crop_bgr, conf=0.7, verbose=False, retina_masks=True)
                if results[0].masks is not None:
                    for mask_tensor in results[0].masks.data:
                        local_mask = cv2.resize(mask_tensor.cpu().numpy(), (self.crop_width, self.crop_height)) > 0.5
                        global_binary_mask[start_y:end_y, :] |= local_mask
...
if __name__ == "__main__":
    ...
    YOLO_WEIGHTS = r"F:\weiyutao\work\ai\hsi\runs\segment\runs\alsi_seg_202606111647_v5\alsi_seg_202606111647_v5\weights\best.pt"
    ...
    #    多数 <2, 中位数 ~0.1), 等价增益 = 7000 / (white-black) ≈ 7000/7000 ≈ 1.0。
    #    若改用【原始 DN】的离线数据, 把 yolo_gains 改回 (7000, 7000, 7000)。
    processor = HSIEdgeProcessor(
        yolo_weights=YOLO_WEIGHTS,
        api_url=API_URL,
        crop_height=480,
        crop_width=640,
        yolo_gains=(1.0, 1.0, 1.0),
        belt_margin=80
    )
